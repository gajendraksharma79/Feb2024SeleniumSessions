...
public class ElementUtil {
	...
	/**
	 * Overloaded method to sendkeys to an element by checking its visibility
	 * 
	 * @param locator
	 * @param timeOut
	 * @param value
	...
	/**
	 * Overloaded method for clicking with Wait
	 * 
	 * @param locator
	 * @param timeOut
	 */
	...
	public void doSearch(By searchField, String searchKey, By suggestions, String clickValue)
	...
	public boolean isElementDisplayed(By locator, int expectedElementCount) {
		...
		if (elementCount == expectedElementCount) {
			...
			System.out.println(
					"multiple or zero elements are displayed " + locator + " with occurrence of " + elementCount);
			return false;
		}
	}
	...
	/**
	 * This method is used to enter the value in text field with a pause of 500 ms
	 * (by default)
	 * 
	 * @param locator
	 * @param value
	...
	public void doSendKeysWithPause(By locator, String value) {
		...
		char ch[] = value.toCharArray();
		for (char c : ch) {
			action.sendKeys(getElement(locator), String.valueOf(c)).pause(500).perform();
		}
	}

	/**
	 * This function is used to handle the 4 level Menu and Submenu
	 * 
	 * @param level1
	 * @param level2
	 * @param level3
	...
	public WebElement waitForElementVisible(By locator, int timeOut, int pollTime) {
	 ...
	 * are visible. Visibility means that the elements are not only displayed but also have
	...
	public String waitForURLContains(String urlFraction, int timeOut) {
		...
		return driver.getCurrentUrl(); // return incorrect/invalid URL
	}
	...
	public String waitForURLToBe(String urlVal, int timeOut) {
		...
		return driver.getCurrentUrl(); // return incorrect/invalid title
	}
	...
	public Alert waitForJSAlert(int timeOut, int pollTime) {
		Wait<WebDriver> wait = new FluentWait<WebDriver>(driver)
									.withTimeout(Duration.ofSeconds(timeOut))
	...
	public void waitForFrameByLocator(By frameLocator, int timeOut, int pollTime) {
		Wait<WebDriver> wait = new FluentWait<WebDriver>(driver)
				.withTimeout(Duration.ofSeconds(timeOut))
				.pollingEvery(Duration.ofSeconds(pollTime))
